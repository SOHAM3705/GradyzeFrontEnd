...
const AttendanceRecords = () => {
  const { classes, attendanceRecords, fetchAttendanceRecords, loading } = useContext(AttendanceContext);
              ...
              {attendanceRecords.map(record => (
                record.records.map((student, index) => (
                  <tr key={`${record._id}-${index}`}>
                    ...
                    <td>{classes.find(c => c._id === record.classId)?.className || 'Unknown'}</td>
                    <td>{student.studentName}</td>
                    <td className={student.status === 'Present' ? 'present-status' : 'absent-status'}>
                      {student.status}
                    </td>
                  </tr>
                ))
